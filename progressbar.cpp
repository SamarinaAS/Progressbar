...
static int xferinfo(void *p, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal,
                    curl_off_t ulnow)
{
    struct progressDownload *myp = (struct progressDownload *)p;
    CURL *curl = myp->curl;
...
class DownloaderGUI : public wxApp
{
public:
    bool OnInit(void);
    int OnExit(void);
...
IMPLEMENT_APP(DownloaderGUI)

bool DownloaderGUI::OnInit()
{
    ...
    Connect(ID_Download, wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(DownloaderGUI::Download));
    download->SetFocus();
    return true;
}

void DownloaderGUI::Download(wxCommandEvent &WXUNUSED(event))
{
...
int DownloaderGUI::OnExit() { return 0; }
